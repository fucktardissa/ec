-- ================== PART 1: LOAD LIBRARIES (Safel 3232 32 32 3y) ==================
...
local RemoteFunction = ReplicatedStorage.Shared.Framework.Network.Remote.RemoteFunction -- From your original script

-- ================== PART 2: DATA, CONFIG, & STATE ==================
local isRerolling = false -- Master state for the reroll process
local Options = Fluent.Options -- Shortcut to access UI values
...
-- This function gets the list of equipped pets AND their IDs
    ...
    for _, petId in ipairs(teamInfo.Pets) do
        ...
        if petInfo then
            ...
            if petInfo.Enchants and next(petInfo.Enchants) then
                local enchantNames = {}
                for _, enchantData in pairs(petInfo.Enchants) do table.insert(enchantNames, enchantData.Id) end
...
-- This function translates a string like "Bubbler V" into a table { id = "bubbler", level = 5 }
local function parseEnchantName(name)
    ...
    if baseName and level then
        ...
        return { id = name:lower():gsub(" ", "-"), level = 1 } -- For enchants without a level
    end
end

-- This function checks if a pet has one of the desired enchants
local function hasDesiredEnchant(petInfo, targetEnchants)
    if not petInfo.Enchants then return false end
...
local Window = Fluent:CreateWindow({
    Title = "Pet Helper", SubTitle = "Enchant Reroller", TabWidth = 160, Size = UDim2.fromOffset(540, 520),
...
local RerollToggle = Tabs.Main:AddToggle("RerollToggle", {
    ...
    Description = "Begin the reroll process for selected pets.",
    Default = false
})

-- ================== PART 5: CORE REROLL & REFRESH LOGIC ==================
local equippedPetsData = getEquippedPetsData() -- Store the full data list

-- Main reroll process, started by the toggle
RerollToggle:OnChanged(function(value)
    isRerolling = value
    if not isRerolling then
        Fluent:Notify({ Title = "Stopped", Content = "Enchant rerolling has been stopped.", Duration = 4 })
        return
    end

    -- This part runs when the toggle is turned ON
    task.spawn(function()
        Fluent:Notify({ Title = "Starting...", Content = "Beginning enchant reroll process.", Duration = 4 })

        -- 1. Get user's selections from the UI
        local selectedPetNames = Options.EquippedPetDropdown.Value
        local selectedEnchantNames = Options.TargetEnchantsDropdown.Value

        -- 2. Translate selections into data the script can use
        local targetPetIds = {}
        ...
        if #targetPetIds == 0 or #targetEnchants == 0 then
            ...
            RerollToggle:SetValue(false) -- Turn toggle off
            isRerolling = false
            return
        end

        -- 3. The Reroll Loop (adapted from your original script)
        while isRerolling do
            ...
            for _, petId in ipairs(targetPetIds) do
                if not isRerolling then break end -- Exit early if stopped mid-cycle

                local petInfo = petDataMap[petId]
                if petInfo and not hasDesiredEnchant(petInfo, targetEnchants) then
                    Fluent:Notify({ Title = "Rerolling", Content = "Attempting reroll for: " .. (petInfo.Name or petId), Duration = 0.5 })
                    RemoteFunction:InvokeServer("RerollEnchants", petId, "Gems")
                    task.wait(0.4) -- Wait between each reroll attempt
                end
            end
            task.wait(0.2) -- Brief pause after checking all pets in the list
        end
    end)
end)

-- Background loop to keep the pet dropdown list up-to-date
task.spawn(function()
