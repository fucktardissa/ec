-- Standalone Auto Mystery Box Script (Continuous & Smart)
...
local Config = {
    ...
    -- The number of each box type to attempt to use.
    USE_QUANTITY = 100,
...
task.spawn(function()
    ...
    while getgenv().Config.AutoOpenAndClaimBoxes do
        ...
        if hasBoxesToOpen then
            ...
            for _, boxName in ipairs(getgenv().Config.BOXES_TO_OPEN) do
                print("-> Attempting to use " .. getgenv().Config.USE_QUANTITY .. "x '" .. boxName .. "'")
                local args = {"UseGift", boxName, getgenv().Config.USE_QUANTITY}
